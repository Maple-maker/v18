...
def find_column_indices(header: List[str]) -> Dict[str, Optional[int]]:
    ...
    indices = {
        ...
        'material': None,
        'auth_qty': None,
        'oh_qty': None,      # On-Hand Quantity - THIS IS WHAT WE USE
        'ui': None,
        'image': None,
    }
    ...
    for i, cell in enumerate(header):
        ...
        # Material column
            ...
            indices['material'] = i
        # On-Hand quantity (THIS IS THE ONE WE USE)
        elif ('OH' in text and 'QTY' in text) or text == 'OH QTY' or text == 'OH':
            indices['oh_qty'] = i
        # Authorized quantity (keep for reference but don't use)
        elif ('AUTH' in text and 'QTY' in text) or text == 'AUTH QTY':
            indices['auth_qty'] = i
        # Unit of Issue (we'll ignore this and always use EA)
...
def extract_items_gcss_standard(tables: List[List[List[str]]]) -> List[BomItem]:
    ...
    - Uses OH Qty (On-Hand) column for quantities
    ...
    for table in tables:
        ...
        for row in table[1:]:
            ...
            # Extract quantity from OH Qty column (On-Hand, not Auth)
            qty = 0  # Default to 0 if not found
            if indices['oh_qty'] is not None and indices['oh_qty'] < len(row):
                qty = extract_quantity(row[indices['oh_qty']])
            
            # Skip items with 0 quantity (not on hand)
            if qty == 0:
                continue
...
def extract_items_epp_format(tables: List[List[List[str]]], page_text: str) -> List[BomItem]:
    ...
    for table in tables:
        ...
        for row in table[1:]:
            ...
            # Extract quantity from OH Qty column (On-Hand)
            qty = 0
            if indices['oh_qty'] is not None and indices['oh_qty'] < len(row):
                qty = extract_quantity(row[indices['oh_qty']])
...
def generate_dd1750_overlay(
    ...
    Args:
        ...
        header: Optional header information to fill in
    ...
    # === HEADER FIELDS ===
    # Field positions based on DD1750 template analysis
    # Using smaller font and text wrapping for longer fields
    
    if header:
        # PACKED BY - field area: x=188-310, two lines available
        if header.packed_by:
            can.setFont("Helvetica", 9)
            packed_by = header.packed_by[:30]  # Limit length
            can.drawString(192, PAGE_H - 58, packed_by)
        
        # NO. BOXES - small field after "1. NO. BOXES"
        if header.num_boxes:
            can.setFont("Helvetica", 10)
            can.drawString(372, PAGE_H - 58, str(header.num_boxes)[:5])
        
        # REQUISITION NO. - field area after "2a."
        if header.requisition_no:
            can.setFont("Helvetica", 8)
            req_no = header.requisition_no[:20]
            can.drawString(497, PAGE_H - 58, req_no)
        
        # ORDER NO. - field area after "2b."
        if header.order_no:
            can.setFont("Helvetica", 8)
            order_no = header.order_no[:20]
            can.drawString(472, PAGE_H - 82, order_no)
        
        # END ITEM - large field, can wrap to two lines
        if header.end_item:
            can.setFont("Helvetica", 8)
            end_item = header.end_item
            # First line (longer)
            line1 = end_item[:55]
            can.drawString(100, PAGE_H - 103, line1)
            # Second line if needed
            if len(end_item) > 55:
                line2 = end_item[55:110]
                can.drawString(50, PAGE_H - 113, line2)
